...
def learn_bpe(inpath, outpath, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False):
    """Learn num_symbols BPE operations from vocabulary, and write to outfile.
    """
    # version 0.2 changes the handling of the end-of-word token ('</w>');
    # version numbering allows bckward compatibility

    outfile = os.path.join(outpath, 'vocab.bpe')
    outfile = codecs.open(outfile, 'w', encoding='utf-8')
    outfile.write('#version: 0.2\n')
    it = util.file_iterator(inpath)

    vocab = get_vocabulary(it, is_dict)
    encoder = {k:i for i,k in enumerate(bytes_to_unicode().values())}
    ...
    for i in range(num_symbols):
        ...
        encoder[''.join(most_frequent)] = len(encoder)
        changes = replace_pair(most_frequent, sorted_vocab, indices)
        update_pair_statistics(most_frequent, changes, stats, indices)
        stats[most_frequent] = 0
        if not i % 100:
            prune_stats(stats, big_stats, threshold)
    encoderfile = os.path.join(outpath,'encoder.json')
    json.dump(encoder,open(encoderfile,'w'))
...
if __name__ == '__main__':
    ...
    #     args.input = csv.reader(open(args.input.name,encoding='utf-8'))
